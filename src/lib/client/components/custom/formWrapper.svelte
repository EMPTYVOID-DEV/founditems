...
<form
	use:enhance={action}
	action={actionName}
	class={cn('flex w-full flex-col rounded-md border-2 border-muted bg-secondary/70', formClass)}
...
>
	...
	<div
		class={cn(
			'flex w-full items-center justify-between border-t-2 border-muted px-4 py-3',
			submitterClass
		)}
	>
		{@render Submitter()}
