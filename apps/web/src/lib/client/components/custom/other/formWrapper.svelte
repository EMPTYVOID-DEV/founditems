...
<form
	...
	action={actionName}
	class={cn(
		'border-foreground bg-foreground/15 flex w-full flex-col rounded-md border-2',
		formClass
	)}
...
>
	...
	<div
		class={cn(
			'border-foreground flex w-full items-center justify-between border-t-2 px-4 py-3',
			submitterClass
		)}
	>
		{@render Submitter()}
